...
@pytest.mark.anyio
async def test_benefits_auths(client: AsyncClient) -> None:
    await authentications(client, f"{API_VERSION}/benefit/")
...
@pytest.mark.anyio
async def test_get_benefits_by_id(client: AsyncClient) -> None:
    access_token: str = await get_admin_token(client)
    for benefit_id in IDS:
        res = await client.get(
            f"{API_VERSION}/benefit/{benefit_id}",
            headers={"Authorization": f"Bearer {access_token}"},
        )
        assert res.status_code == status.HTTP_200_OK, res.read()
        res_data: dict = res.json()
        assert res_data["id"] == benefit_id


@pytest.mark.anyio
async def test_update_benefits(client: AsyncClient) -> None:
    access_token: str = await get_admin_token(client)
...
@pytest.mark.anyio
async def test_delete_benefits(client: AsyncClient) -> None:
    access_token: str = await get_admin_token(client)
    for benefit_id in IDS:
